...
test('Mid-cycle upgrade applies prorated credit and upgrades plan', async ({ page }) => {
  // Create a fresh Free-plan customer
  let {intpcId, intpWebsiteId} = await createRandomCustomer()

  // Step 1: Go to dashboard
  await openDashboard(page, intpcId, intpWebsiteId);

  // Step 2: Start upgrade from dashboard to Basic plan
  ...
  await upgradePlan(
    page,
    'Advanced',
    'Advanced 27,500 Monthly Page Views',
    '€24.99Monthly'
  );
  ...
  await page.getByRole("button").nth(2).click(); // Close the modal
